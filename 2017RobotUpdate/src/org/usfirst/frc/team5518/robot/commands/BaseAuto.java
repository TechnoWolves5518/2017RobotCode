...
//					System.out.println("!!!!!!!!!!!!!!!!!DOORS ALREADY CLOSED!!!!!!!!!!!!!!!!!");
		...
		*/
		...
		if (movingForward && avg != 1000) {
			speed = (Math.pow((avg-7), 0.5)) / 11; // Slows down then stops at 7 inches
			if (speed > 0.5) {
				speed = 0.5;
			}
		...
		else if (movingForward == false && avg != 1000) {
			if (doorsOpen = false) {
				Robot.motorController.openDoors();
				doorsOpen = true;
			}
			speed = avg / 30; // Should start moving back at a speed of 0.233
			if (speed > 0.5) { //switch to 0.5
				speed = 0.5;
			}
		}
